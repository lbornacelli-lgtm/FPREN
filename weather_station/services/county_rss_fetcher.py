...
def _fetch_county(county, zone):
    ...
    try:
        ...
        logger.warning("%s: error: %s", county, e)
        return []
    docs = []
...
def run_once(db):
    ...
    for county, zone in FLORIDA_COUNTIES:
        for doc in _fetch_county(county, zone):
            alert_id = doc["alert_id"]
            existing = col.find_one({"alert_id": alert_id})
            if existing:
                changed = any(existing.get(k) != doc[k] for k in ("event","headline","severity"))
                if changed:
                    col.update_one({"alert_id": alert_id}, {"$set": {**doc, "tts_generated": False}})
                    upserted += 1
            else:
