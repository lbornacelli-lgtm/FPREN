...
def apply_audio_chain(input_file: str, output_file: str):
    ...
    board = Pedalboard([
        ...
        HighShelfFilter(cutoff_frequency_hz=10000, gain_db=4.0), # Add air
        LowShelfFilter(cutoff_frequency_hz=120, gain_db=3.0),    # Add warmth
        Reverb(room_size=0.3, wet_level=0.2),    # Subtle reverb
        Delay(delay_seconds=0.25, feedback=0.2, mix=0.15)  # Slight echo
    ])
